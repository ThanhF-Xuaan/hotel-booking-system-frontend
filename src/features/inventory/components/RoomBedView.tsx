...
import { parseDimensionString } from '../../../utils/dimension';
import { RoomBedResponse } from '../../../types/inventory';
...
const RoomBedView: React.FC = () => {
  const [items, setItems] = useState<RoomBedResponse[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<RoomBedResponse | null>(null);

  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<RoomBedResponse | null>(null);
  ...
  const fetchRoomBeds = async (): Promise<void> => {
    setIsLoading(true);
    setFetchError(null);
    try {
      const data = await api.get('/hotel/api/v1/inventory/room-beds');
      setItems(Array.isArray(data) ? (data as RoomBedResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch room beds from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (bed: RoomBedResponse): void => {
    const bedSizeStr = (bed as unknown as { size?: string }).size || '';
    const parsed = parseDimensionString(bedSizeStr) as { width: number; height: number } | null;
    setEditingItem(bed);
    setFormData({
      name: bed.name,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save room bed.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (bed: RoomBedResponse): void => {
    setItemToDelete(bed);
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete room bed.');
    } finally {
  ...
  return (
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-black tracking-tight">Bed Configurations</h2>
          <p className="text-sm text-neutral-500">Configure bed sizes and custom mappings for room allocation.</p>
        </div>
        <button
          type="button"
          ...
          aria-label="Add new bed configuration"
        >
          <Plus className="w-5 h-5" />
          <span>Add Bed Config</span>
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-neutral-900 text-white uppercase text-[11px] font-bold tracking-wider">
                ...
                <th className="py-4 px-6">Bed Configuration</th>
                <th className="py-4 px-6">Physical Size</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-neutral-500 font-medium bg-neutral-50">
                    No bed configurations registered. Click "Add Bed Config" to create one.
                  </td>
                </tr>
              ) : (
                items.map((bed) => (
                  <tr key={bed.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{bed.id}</td>
                    <td className="py-4 px-6 font-semibold text-black">{bed.name}</td>
                    <td className="py-4 px-6 font-mono text-neutral-600">{(bed as unknown as { size?: string }).size}</td>
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <div className="bg-neutral-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="text-lg font-bold uppercase tracking-wider">
                {editingItem ? 'Edit Bed Config' : 'Add Bed Config'}
              </h3>
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-neutral-450 mt-1 leading-normal">
                  Note: Enter values in meters (e.g. 1.8). If values &gt; 10 are typed (e.g. 180), they automatically resolve from centimeters to meters on focus blur.
                </p>
              </div>
