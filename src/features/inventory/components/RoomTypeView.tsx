...
import { RoomTypeResponse } from '../../../types/inventory';
...
const RoomTypeView: React.FC = () => {
  ...
  const fetchRoomTypes = async (): Promise<void> => {
    ...
    setFetchError(null);
    try {
      const data = await api.get('/hotel/api/v1/inventory/room-types');
      setItems(Array.isArray(data) ? (data as RoomTypeResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch room types from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (type: RoomTypeResponse): void => {
    setEditingItem(type);
    setFormData({
      code: type.code,
      name: type.name,
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.code.trim()) {
      newErrors.code = 'Room type code is required';
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (editingItem) {
        // PUT Request
        await api.put(`/hotel/api/v1/inventory/room-types/${editingItem.id}`, payload);
      } else {
        // POST Request
        await api.post('/hotel/api/v1/inventory/room-types', payload);
      }
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save room type.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (type: RoomTypeResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    setFetchError(null);
    try {
      await api.delete(`/hotel/api/v1/inventory/room-types/${itemToDelete.id}`);
      fetchRoomTypes();
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete room type.');
    } finally {
  ...
  return (
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-black tracking-tight">Room Types Catalog</h2>
          <p className="text-sm text-neutral-500">Configure global room classifications, room codes, and properties.</p>
        </div>
        <button
          type="button"
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-neutral-900 text-white uppercase text-[11px] font-bold tracking-wider">
                ...
                <th className="py-4 px-6">Room Type Code</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-neutral-500 font-medium bg-neutral-50">
                    No global room types configured. Click "Add Room Type" to create one.
                  </td>
                </tr>
              ) : (
                items.map((type) => (
                  <tr key={type.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{type.id}</td>
                    <td className="py-4 px-6 font-mono font-bold text-red-600">{type.code}</td>
                    <td className="py-4 px-6 font-semibold text-black">{type.name}</td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={`Edit room type ${type.name}`}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {/* Code */}
              <div>
                <label htmlFor="type-code" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Room Type Code *
                </label>
                <input
                  type="text"
              ...
              {/* Name */}
              <div>
                <label htmlFor="type-name" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Room Type Name *
                </label>
                <input
                  type="text"
              ...
              {/* Status */}
              <div>
                <label htmlFor="type-status" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Status
                </label>
                <select
                  id="type-status"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-[1px] animate-fade-in">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center border border-neutral-200 z-10 animate-scale-up">
            ...
            <p className="text-sm text-neutral-600 mb-6">
              Are you sure you want to permanently delete room type classification: <strong className="text-neutral-900 font-bold">{itemToDelete?.name}</strong>? This action cannot be undone.
            </p>
            <div className="flex items-center space-x-3">
              <button
                type="button"
