...
const RoomFeatureView = () => {
  ...
  const fetchRoomFeatures = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch room features from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setSubmitError(err.response?.data?.message || 'Failed to save room feature.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete room feature.');
    } finally {
    ...
    <div className="relative space-y-6 min-h-[400px]">
      ...
      {isLoading && (
        <div className="absolute inset-0 z-50 bg-white/70 backdrop-blur-[2px] flex items-center justify-center rounded-lg">
          <div className="flex flex-col items-center space-y-2">
            ...
            <span className="text-xs font-bold text-neutral-900 tracking-wider uppercase">Loading Features...</span>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500">Configure catalog room items, views, and structural properties.</p>
        </div>
        <button
              ...
              {items.length === 0 ? (
                ...
                items.map((feat) => (
                  <tr key={feat.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-950">{feat.id}</td>
                    <td className="py-4 px-6 font-mono font-bold text-red-600">{feat.code}</td>
                    <td className="py-4 px-6 font-semibold text-black">{feat.name}</td>
                    <td className="py-4 px-6 text-neutral-600 font-mono">{feat.icon || '—'}</td>
                    <td className="py-4 px-6">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-neutral-100 text-neutral-900 border border-neutral-300">
                        {feat.category}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={`Edit feature ${feat.code}`}
                        >
                        ...
                        <button
                          ...
                          aria-label={`Delete feature ${feat.code}`}
                        >
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <div className="bg-neutral-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="text-lg font-bold uppercase tracking-wider">
                {editingItem ? 'Edit Feature Info' : 'Add New Feature'}
              </h3>
              <button
                onClick={() => setIsModalOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {/* Feature Code */}
              <div>
                <label htmlFor="feature-code" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Feature Code *
                </label>
                <input
                  type="text"
              ...
              {/* Feature Name */}
              <div>
                <label htmlFor="feature-name" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Feature Name *
                </label>
                <input
                  type="text"
              ...
              {/* Icon Key */}
              <div>
                <label htmlFor="feature-icon" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Icon Identifier
                </label>
                <input
                  type="text"
                  ...
                  placeholder="E.g., wind, bath, tv"
                />
              </div>

              {/* Category Dropdown */}
              <div>
                <label htmlFor="feature-category" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Category *
                </label>
                <select
                  id="feature-category"
              ...
              {/* Status */}
              <div>
                <label htmlFor="feature-status" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Status
                </label>
                <select
                  id="feature-status"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-[1px] animate-fade-in">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center border border-neutral-200 z-10 animate-scale-up">
            ...
            <p className="text-sm text-neutral-600 mb-6">
              Are you sure you want to permanently delete feature: <strong className="text-neutral-900 font-bold">{itemToDelete?.code}</strong>? This action cannot be undone.
            </p>
            <div className="flex items-center space-x-3">
              <button
                type="button"
