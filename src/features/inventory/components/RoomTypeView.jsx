...
const RoomTypeView = () => {
  ...
  const fetchRoomTypes = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch room types from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setSubmitError(err.response?.data?.message || 'Failed to save room type.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete room type.');
    } finally {
  ...
  return (
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-black tracking-tight">Room Types</h2>
          <p className="text-sm text-neutral-500">Configure catalog options and room classifications.</p>
        </div>
        <button
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan="5" className="py-8 text-center text-neutral-500 font-medium bg-neutral-50">
                    No room types registered. Click "Add Room Type" to create one.
                  </td>
                </tr>
              ) : (
                items.map((type) => (
                  <tr key={type.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-950">{type.id}</td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={`Edit room type ${type.code}`}
                        >
                        ...
                        <button
                          ...
                          aria-label={`Delete room type ${type.code}`}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {/* Room Type Code */}
              <div>
                <label htmlFor="room-type-code" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Type Code *
                </label>
                <input
                  type="text"
              ...
              {/* Room Type Name */}
              <div>
                <label htmlFor="room-type-name" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Type Name *
                </label>
                <input
                  type="text"
              ...
              {/* Status */}
              <div>
                <label htmlFor="room-type-status" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Status
                </label>
                <select
                  id="room-type-status"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-[1px] animate-fade-in">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center border border-neutral-200 z-10 animate-scale-up">
            ...
            <p className="text-sm text-neutral-600 mb-6">
              Are you sure you want to permanently delete room type: <strong className="text-neutral-900 font-bold">{itemToDelete?.code}</strong>? This action cannot be undone.
            </p>
            <div className="flex items-center space-x-3">
              <button
                type="button"
