...
import { HotelResponse } from '../../../types/inventory';
...
const HotelView: React.FC = () => {
  const [items, setItems] = useState<HotelResponse[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<HotelResponse | null>(null);

  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<HotelResponse | null>(null);
  ...
  const fetchHotels = async (): Promise<void> => {
    ...
    try {
      ...
      setItems(Array.isArray(data) ? (data as HotelResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch hotels from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (hotel: HotelResponse): void => {
    setEditingItem(hotel);
    // Slice seconds (e.g. "14:00:00" to "14:00") for HTML time inputs
    const checkInTimeStr = (hotel as unknown as { checkInTime?: string }).checkInTime || '14:00';
    const checkOutTimeStr = (hotel as unknown as { checkOutTime?: string }).checkOutTime || '12:00';
    const formattedCheckIn = checkInTimeStr.substring(0, 5);
    const formattedCheckOut = checkOutTimeStr.substring(0, 5);
    ...
    setFormData({
      ...
      serviceFeePercent: (hotel as unknown as { serviceFeePercent?: number }).serviceFeePercent?.toString() || '5.00',
      status: hotel.status
    });
    setErrors({});
    setSubmitError(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save hotel.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (hotel: HotelResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete hotel.');
    } finally {
  ...
  return (
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-black tracking-tight">Hotel Locations</h2>
          <p className="text-sm text-neutral-500">Configure branch networks, default check-in rules, and operational service charges.</p>
        </div>
        <button
          type="button"
          ...
          aria-label="Add new hotel location"
        >
          <Plus className="w-5 h-5" />
          <span>Add Hotel Location</span>
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-neutral-900 text-white uppercase text-[11px] font-bold tracking-wider">
                ...
                <th className="py-4 px-6">Branch Address</th>
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-8 text-center text-neutral-500 font-medium bg-neutral-50">
                    No hotel branches registered. Click "Add Hotel Location" to create one.
                  </td>
                </tr>
              ) : (
                items.map((hotel) => (
                  <tr key={hotel.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{hotel.id}</td>
                    <td className="py-4 px-6 font-semibold text-black">{hotel.name}</td>
                    <td className="py-4 px-6 text-neutral-600">{hotel.address}</td>
                    <td className="py-4 px-6 text-neutral-600 font-medium font-mono">{hotel.phone}</td>
                    <td className="py-4 px-6 font-mono font-medium text-neutral-600 text-xs">
                      {((hotel as unknown as { checkInTime?: string }).checkInTime || '14:00').substring(0, 5)} / {((hotel as unknown as { checkOutTime?: string }).checkOutTime || '12:00').substring(0, 5)}
                    </td>
                    <td className="py-4 px-6 text-neutral-950 font-bold font-mono text-xs">
                      {(hotel as unknown as { serviceFeePercent?: number }).serviceFeePercent}%
                    </td>
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <div className="bg-neutral-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="text-lg font-bold uppercase tracking-wider">
                {editingItem ? 'Edit Hotel' : 'Add Hotel'}
              </h3>
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="hotel-checkin" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                    Check-In Time *
                  </label>
                  <input
                    type="time"
                ...
                <div>
                  <label htmlFor="hotel-checkout" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                    Check-Out Time *
                  </label>
                  <input
                    type="time"
              ...
              {/* Service Fee */}
              <div>
                <label htmlFor="hotel-fee" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Service Charge Fee (%) *
                </label>
                <input
                  type="number"
              ...
              {/* Status */}
              <div>
                <label htmlFor="hotel-status" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Status
                </label>
                <select
                  id="hotel-status"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-[1px] animate-fade-in">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center border border-neutral-200 z-10 animate-scale-up">
            ...
            <p className="text-sm text-neutral-600 mb-6">
              Are you sure you want to permanently delete hotel location: <strong className="text-neutral-900 font-bold">{itemToDelete?.name}</strong>? This action cannot be undone.
            </p>
            <div className="flex items-center space-x-3">
              <button
                type="button"
