...
const RoomInstancePage: React.FC = () => {
  ...
  const [hotelRoomTypes, setHotelRoomTypes] = useState<HotelRoomTypeResponse[]>([]);
  const [roomInstances, setRoomInstances] = useState<RoomInstanceResponse[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<RoomInstanceResponse | null>(null);

  // Create/Edit Room Modal State
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<RoomInstanceResponse | null>(null);
  const [errors, setErrors] = useState<FormErrors>({});

  // Quick Status Modal State
  const [isStatusModalOpen, setIsStatusModalOpen] = useState<boolean>(false);
  const [statusEditingItem, setStatusEditingItem] = useState<RoomInstanceResponse | null>(null);
  const [selectedStatus, setSelectedStatus] = useState<RoomInstanceResponse['currentStatus']>('READY');
  ...
  const fetchConfigData = async (): Promise<void> => {
    ...
    try {
      ...
      setGlobalRoomTypes(Array.isArray(globalTypesData) ? (globalTypesData as RoomTypeResponse[]) : []);
      ...
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch setup configurations from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchHotelData = async (hotelId: string): Promise<void> => {
    if (!hotelId) return;
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      setRoomInstances(Array.isArray(instancesData) ? (instancesData as RoomInstanceResponse[]) : []);
      setHotelRoomTypes(Array.isArray(typesConfigData) ? (typesConfigData as HotelRoomTypeResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to load room details for the selected hotel.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (room: RoomInstanceResponse): void => {
    setEditingItem(room);
    setFormData({
  ...
  const handleOpenStatusEdit = (room: RoomInstanceResponse): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await roomInstanceApi.update(editingItem.id, updatePayload);
      } else {
        // Create Room (hotelId and currentStatus required on create)
        const createPayload = {
          ...payload,
          hotelId: parseInt(selectedHotelId, 10),
          currentStatus: 'READY' as const // default to READY on creation
        };
        await roomInstanceApi.create(createPayload);
      }
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save room details.');
    } finally {
      setIsLoading(false);
    }
  };

  const handleStatusSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setStatusSubmitError(axiosError.response?.data?.message || 'Failed to update room status.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (room: RoomInstanceResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete room.');
    } finally {
  ...
  const getStatusBadgeStyles = (status: RoomInstanceResponse['currentStatus']): string => {
    switch (status) {
      case 'READY':
      ...
      case 'CLEANING':
        return 'bg-neutral-900 text-white border border-neutral-955';
      case 'MAINTENANCE':
    ...
    <div className="relative space-y-6 min-h-[400px]">
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-neutral-100 text-neutral-700 hover:text-black focus:outline-none focus:ring-2 focus:ring-red-605"
                        ...
                        <button
                          ...
                          className="p-1.5 rounded hover:bg-red-50 text-neutral-600 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-red-605"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden border border-neutral-200 z-10 animate-scale-up">
            {/* Modal Header */}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-neutral-400 mt-1 font-medium">Accepts alphanumeric and underscores (e.g. 101, RM_202). Length: 3-20 characters.</p>
              </div>
                ...
                >
                  ...
                  {hotelRoomTypes.map((t) => (
                    <option key={t.id} value={t.id.toString()}>
                      {getRoomTypeName(t.id)} (Price: {parseFloat(t.basePrice.toString()).toLocaleString()} VND)
                    </option>
                  ))}
                </select>
            ...
            <form onSubmit={handleStatusSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSelectedStatus(e.target.value as RoomInstanceResponse['currentStatus'])}
