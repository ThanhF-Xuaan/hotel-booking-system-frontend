...
const StaffPage: React.FC = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<StaffResponse | null>(null);

  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<StaffResponse | null>(null);
  ...
  const fetchData = async (): Promise<void> => {
    ...
    try {
      ...
      setHotels(Array.isArray(hotelsData) ? (hotelsData as DropdownItem[]) : []);
      setRoles(Array.isArray(rolesData) ? (rolesData as RoleResponse[]) : []);
      setStaffs(Array.isArray(staffsData) ? (staffsData as StaffResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to initialize staff details from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (staff: StaffResponse): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setStaffs(Array.isArray(updatedStaffs) ? (updatedStaffs as StaffResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save staff member.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (staff: StaffResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete staff member.');
    } finally {
              ...
              {staffs.length === 0 ? (
                ...
                staffs.map((staff) => (
                  <tr key={staff.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{staff.id}</td>
              ...
              {submitError && (
                <div className="bg-red-50 border-l-4 border-red-600 p-4 rounded flex items-start space-x-3 shadow-sm">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-red-755 mt-0.5">{submitError}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-neutral-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-red-650"
                    placeholder="e.g. Doe"
                  />
                </div>
