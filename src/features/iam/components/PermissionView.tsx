...
import { PermissionResponse } from '../../../types/iam';
...
const PermissionView: React.FC = () => {
  const [items, setItems] = useState<PermissionResponse[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<PermissionResponse | null>(null);

  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<PermissionResponse | null>(null);
  ...
  const fetchPermissions = async (): Promise<void> => {
    ...
    try {
      ...
      setItems(Array.isArray(data) ? (data as PermissionResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch permissions from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (permission: PermissionResponse): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save permission.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (permission: PermissionResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete permission.');
    } finally {
    ...
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500">Configure Action & Resource mappings for access control.</p>
        </div>
        <button
          type="button"
          ...
          aria-label="Create new permission mapping"
        >
              ...
              {items.length === 0 ? (
                ...
                items.map((permission) => (
                  <tr key={permission.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{permission.id}</td>
                    <td className="py-4 px-6 font-mono font-bold text-red-600">{permission.action}</td>
                    <td className="py-4 px-6 font-semibold text-black">{permission.resource}</td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={`Edit permission ${permission.action}:${permission.resource}`}
                        >
                        ...
                        <button
                          ...
                          aria-label={`Delete permission ${permission.action}:${permission.resource}`}
                        >
          ...
          <div className="relative bg-white border border-neutral-200 rounded-lg shadow-xl w-full max-w-md z-10 overflow-hidden animate-scale-up">
            ...
            <div className="bg-neutral-900 text-white px-6 py-4 flex items-center justify-between">
              <h3 className="text-lg font-bold uppercase tracking-wider">
                {editingItem ? 'Edit Permission' : 'Create Permission'}
              </h3>
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {/* Action */}
              <div>
                <label htmlFor="action" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Action Code *
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full px-3 py-2 border rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-red-600
                    ${errors.action ? 'border-red-600 focus:border-red-600' : 'border-neutral-300 focus:border-black'}
                  `}
              ...
              {/* Resource */}
              <div>
                <label htmlFor="resource" className="block text-xs font-bold uppercase text-neutral-900 tracking-wider mb-1">
                  Resource Code *
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full px-3 py-2 border rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-red-600
                    ${errors.resource ? 'border-red-600 focus:border-red-600' : 'border-neutral-300 focus:border-black'}
                  `}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-[1px] animate-fade-in">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 text-center border border-neutral-200 z-10 animate-scale-up">
            ...
            <p className="text-sm text-neutral-600 mb-6">
              Are you sure you want to permanently delete permission mapping: <strong className="text-neutral-900 font-bold">{itemToDelete?.action}:{itemToDelete?.resource}</strong>? This action cannot be undone.
            </p>
            <div className="flex items-center space-x-3">
              <button
                type="button"
