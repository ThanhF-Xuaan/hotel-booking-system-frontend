...
import { GuestResponse } from '../../../types/crm';
...
const GuestView: React.FC = () => {
  const [items, setItems] = useState<GuestResponse[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<GuestResponse | null>(null);

  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<GuestResponse | null>(null);
  ...
  const fetchGuests = async (): Promise<void> => {
    ...
    try {
      ...
      setItems(Array.isArray(data) ? (data as GuestResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch guests from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (guest: GuestResponse): void => {
  ...
  const handleNameChange = (field: 'firstName' | 'lastName', val: string): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save guest.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (guest: GuestResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete guest.');
    } finally {
