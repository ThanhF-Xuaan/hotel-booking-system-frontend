...
export interface PricingRuleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (payload: PricingRuleCreateRequest | PricingRuleUpdateRequest) => Promise<void>;
  editingItem: PricingRuleResponse | null;
}
...
const PricingRuleModal: React.FC<PricingRuleModalProps> = ({ isOpen, onClose, onSave, editingItem }) => {
  ...
  const [ruleTypes, setRuleTypes] = useState<RuleTypeItem[]>([]);
  ...
  const fetchOptionsData = async (): Promise<void> => {
    ...
    try {
      ...
      setHolidays(Array.isArray(holidaysList) ? (holidaysList as HolidayCalendarItem[]) : []);
      setRuleTypes(Array.isArray(ruleTypesList) ? (ruleTypesList as RuleTypeItem[]) : []);
    } catch (err) {
          ...
          {submitError && (
            <div className="bg-red-50 border-l-4 border-red-600 p-4 rounded flex items-start space-x-3 shadow-sm animate-fade-in">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-red-707 mt-0.5">{submitError}</p>
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-red-600 font-mono font-bold
                  ${errors.adjustmentValue ? 'border-red-655' : 'border-neutral-300'}
                `}
