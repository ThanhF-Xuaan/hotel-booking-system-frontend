...
const PricingRulePage: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<PricingRuleResponse | null>(null);

  // Custom Delete Modal State
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
  const [itemToDelete, setItemToDelete] = useState<PricingRuleResponse | null>(null);
  ...
  const fetchSetupData = async (): Promise<void> => {
    ...
    try {
      ...
      setFetchError(axiosError.response?.data?.message || 'Failed to retrieve baseline configuration directories.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Fetch pricing rules
  const fetchRules = async (roomTypeId: string): Promise<void> => {
    setIsLoading(true);
    setFetchError(null);
    try {
      const data = await pricingRuleApi.getAll(roomTypeId ? parseInt(roomTypeId, 10) : undefined);
      setRules(Array.isArray(data) ? (data as PricingRuleResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch pricing rules list.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (rule: PricingRuleResponse): void => {
    setEditingItem(rule);
    setIsModalOpen(true);
  };

  const handleSave = async (payload: PricingRuleCreateRequest | PricingRuleUpdateRequest): Promise<void> => {
    if (editingItem) {
      // Update
      const result = await pricingRuleApi.update(editingItem.id, payload as PricingRuleUpdateRequest);
      setRules(prev => prev.map(r => r.id === editingItem.id ? (result as PricingRuleResponse) : r));
    } else {
      // Create
      const result = await pricingRuleApi.create(payload as PricingRuleCreateRequest);
      setRules(prev => [result as PricingRuleResponse, ...prev]);
    }
    setIsModalOpen(false);
    setEditingItem(null);
  };

  const handleOpenDelete = (rule: PricingRuleResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete pricing rule.');
    } finally {
      setIsLoading(false);
    }
  };
