...
const PricingRuleTypePage = () => {
  ...
  const fetchPricingRuleTypes = async () => {
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch pricing rule types.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (payload) => {
    if (editingItem) {
      ...
      const result = await pricingRuleTypeApi.update(editingItem.code, payload);
      setTypes(prev => prev.map(t => t.code === editingItem.code ? result : t));
    } else {
      // create new endpoint
      const result = await pricingRuleTypeApi.create(payload);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete pricing rule type config.');
      setIsDeleteModalOpen(false);
    } finally {
      setIsLoading(false);
    }
