...
export interface PricingRuleTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (payload: RuleTypeCreateRequest | RuleTypeUpdateRequest) => Promise<void>;
  editingItem: RuleTypeResponse | null;
}
...
const PricingRuleTypeModal: React.FC<PricingRuleTypeModalProps> = ({ isOpen, onClose, onSave, editingItem }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    let payload: RuleTypeCreateRequest | RuleTypeUpdateRequest;
    if (!editingItem) {
      payload = {
    ...
    try {
      ...
      setSubmitError(axiosError.response?.data?.message || 'Failed to save pricing rule type configuration.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-white border border-neutral-200 rounded-lg shadow-2xl w-full max-w-md z-10 overflow-hidden animate-scale-in">
        ...
        <div className="bg-neutral-900 text-white px-6 py-4 flex items-center justify-between">
          <h3 className="text-lg font-bold uppercase tracking-wider">
            {editingItem ? 'Edit Pricing Rule Type' : 'Create Pricing Rule Type'}
          </h3>
          <button 
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. SPECIAL_HOLIDAY"
              className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-red-600
                ${editingItem 
                  ? 'bg-neutral-100 text-neutral-500 cursor-not-allowed border-neutral-200 font-semibold' 
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Special Holiday Adjustment"
              className={`w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-2 focus:ring-red-600
...
export default PricingRuleTypeModal;
