...
const SurchargeRulePage = () => {
  ...
  const fetchInitializationData = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to load master configuration listings.');
    } finally {
      setIsLoading(false);
    }
  };

  const fetchSurchargeRules = async (roomTypeId) => {
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch surcharge rules.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (payload) => {
    if (editingItem) {
      const result = await surchargeRuleApi.update(editingItem.id, payload);
      setRules(prev => prev.map(r => r.id === editingItem.id ? result : r));
    } else {
      const result = await surchargeRuleApi.create(payload);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete surcharge rule.');
    } finally {
      setIsLoading(false);
    }
  };
