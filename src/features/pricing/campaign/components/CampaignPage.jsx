...
const CampaignPage = () => {
  ...
  const fetchHotels = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch hotels directory.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchCampaigns = async (hotelId) => {
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to retrieve campaigns list.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (payload) => {
    if (editingItem) {
      const result = await campaignApi.update(editingItem.id, payload);
      setCampaigns(prev => prev.map(c => c.id === editingItem.id ? result : c));
    } else {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete campaign.');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {campaigns.length === 0 ? (
                ...
                campaigns.map((c) => (
                  <tr key={c.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-950">{c.id}</td>
