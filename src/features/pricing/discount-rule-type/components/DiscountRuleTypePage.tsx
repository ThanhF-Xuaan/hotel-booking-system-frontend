...
const DiscountRuleTypePage: React.FC = () => {
  ...
  const fetchDiscountRuleTypes = async (): Promise<void> => {
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch discount rule types.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (payload: RuleTypeCreateRequest | RuleTypeUpdateRequest): Promise<void> => {
    if (editingItem) {
      ...
      const result = await discountRuleTypeApi.update(editingItem.code, payload as RuleTypeUpdateRequest);
      setTypes(prev => prev.map(t => t.code === editingItem.code ? (result as RuleTypeResponse) : t));
    } else {
      // create new endpoint
      const result = await discountRuleTypeApi.create(payload as RuleTypeCreateRequest);
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete discount rule type config.');
      setIsDeleteModalOpen(false);
    } finally {
      setIsLoading(false);
    }
