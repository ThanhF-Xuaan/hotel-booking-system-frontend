...
import { discountRuleTypeApi } from '../api/discountRuleTypeApi';
import DiscountRuleTypeModal from './DiscountRuleTypeModal';

const DiscountRuleTypePage = () => {
  ...
  const fetchDiscountRuleTypes = async () => {
    setIsLoading(true);
    setFetchError(null);
    try {
      const data = await discountRuleTypeApi.getAll();
      setTypes(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch discount rule types.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (payload) => {
    if (editingItem) {
      ...
      const result = await discountRuleTypeApi.update(editingItem.code, payload);
      setTypes(prev => prev.map(t => t.code === editingItem.code ? result : t));
    } else {
      // create new endpoint
      const result = await discountRuleTypeApi.create(payload);
      setTypes(prev => [...prev, result].sort((a, b) => b.priority - a.priority));
    }
    setIsModalOpen(false);
    setEditingItem(null);
  };

  const handleOpenDelete = (typeObj) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsLoading(true);
    try {
      await discountRuleTypeApi.delete(itemToDelete.code);
      setTypes(prev => prev.filter(t => t.code !== itemToDelete.code));
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
    } catch (err) {
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete discount rule type config.');
      setIsDeleteModalOpen(false);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="relative space-y-6 min-h-[400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-black tracking-tight">Discount Rule Types</h2>
          <p className="text-sm text-neutral-500">Configure promotional types, precedence ordering, and classification for booking discounts.</p>
        </div>
        ...
        <button
          ...
          aria-label="Create new discount rule type"
        >
              ...
              {types.length === 0 ? (
                <tr>
                  <td colSpan="4" className="py-8 text-center text-neutral-500 font-medium bg-neutral-50">
                    No discount rule types configured.
                  </td>
                </tr>
              ) : (
                types.map((type) => (
                  <tr key={type.code} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-mono font-bold text-neutral-950">{type.code}</td>
                    ...
                    <td className="py-4 px-6 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end space-x-2">
                        <button
                          ...
                          aria-label={`Edit discount rule type ${type.code}`}
                        >
                        ...
                        <button
                          ...
                          aria-label={`Delete discount rule type ${type.code}`}
                        >
      ...
      {isDeleteModalOpen && itemToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden border border-neutral-200 z-10 animate-scale-in p-6 text-center">
            ...
            <h4 className="text-lg font-extrabold text-black uppercase tracking-tight mb-2">Delete Discount Rule Type</h4>
...
export default DiscountRuleTypePage;
