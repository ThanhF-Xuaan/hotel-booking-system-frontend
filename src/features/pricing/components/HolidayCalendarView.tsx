...
import { HolidayCalendarResponse } from '../../../types/pricing';
...
const HolidayCalendarView: React.FC = () => {
  const [items, setItems] = useState<HolidayCalendarResponse[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [fetchError, setFetchError] = useState<string | null>(null);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [editingItem, setEditingItem] = useState<HolidayCalendarResponse | null>(null);
  const [errors, setErrors] = useState<FormErrors>({});

  // Custom Delete Modal State
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
  const [itemToDelete, setItemToDelete] = useState<HolidayCalendarResponse | null>(null);
  ...
  const fetchHolidays = async (): Promise<void> => {
    ...
    try {
      ...
      setItems(Array.isArray(data) ? (data as HolidayCalendarResponse[]) : []);
    } catch (err: unknown) {
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to fetch holiday calendars from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (holiday: HolidayCalendarResponse): void => {
    setEditingItem(holiday);
    setFormData({
      name: holiday.name,
      date: holiday.date || '',
      description: (holiday as unknown as { description?: string }).description || '',
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setSubmitError(axiosError.response?.data?.message || 'Failed to save holiday calendar.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenDelete = (holiday: HolidayCalendarResponse): void => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      const axiosError = err as { response?: { data?: { message?: string } } };
      setFetchError(axiosError.response?.data?.message || 'Failed to delete holiday calendar.');
    } finally {
              ...
              {items.length === 0 ? (
                ...
                items.map((holiday) => (
                  <tr key={holiday.id} className="hover:bg-neutral-50 transition-colors">
                    <td className="py-4 px-6 font-bold text-neutral-955">{holiday.id}</td>
                    ...
                    <td className="py-4 px-6 text-neutral-600 max-w-xs truncate">
                      {(holiday as unknown as { description?: string }).description || <span className="text-neutral-400 italic">None</span>}
                    </td>
