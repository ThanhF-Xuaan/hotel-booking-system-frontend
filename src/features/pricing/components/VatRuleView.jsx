...
const VatRuleView = () => {
  ...
  const fetchVatRules = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch VAT rules from the server.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setSubmitError(err.response?.data?.message || 'Failed to save VAT rule.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete VAT rule.');
    } finally {
              ...
              {items.length === 0 ? (
                ...
                items.map((rule) => (
                  <tr key={rule.id} className="hover:bg-neutral-50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-neutral-950 font-extrabold">{rule.vatPercent}%</td>
