...
export interface DiscountRuleModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (payload: DiscountRuleCreateRequest | DiscountRuleUpdateRequest) => Promise<void>;
  editingItem: DiscountRuleResponse | null;
}
...
const DiscountRuleModal: React.FC<DiscountRuleModalProps> = ({ isOpen, onClose, onSave, editingItem }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Create Mode fields
    appliedRoomTypeIds: [] as number[],
    tiers: [] as DiscountTierRequest[],
  ...
  const fetchInitializationData = async (): Promise<void> => {
    ...
    try {
      ...
      setRuleTypes(Array.isArray(ruleTypesList) ? (ruleTypesList as RuleTypeConfig[]) : []);
    } catch (err) {
  ...
  const handleRoomTypeCheckboxChange = (id: number): void => {
    setFormData(prev => {
  ...
  const handleTierChange = (index: number, field: 'discountType' | 'discountValue', value: string): void => {
    setFormData(prev => {
  ...
  const handleTierConditionChange = (index: number, field: keyof DiscountCondition, value: string): void => {
    setFormData(prev => {
      const updatedTiers = [...prev.tiers];
      const parsedVal = (field === 'promoCode') 
        ? (value || null) 
  ...
  const validateForm = (): boolean => {
    ...
    if (!editingItem) {
      ...
      formData.tiers.forEach((tier, idx) => {
        const errorsForTier: Record<string, string> = {};
        if (tier.discountValue <= 0) {
          errorsForTier.discountValue = 'Must be greater than 0';
        }

        if (formData.ruleTypeCode === 'LONG_STAY') {
          const nights = tier.conditions.minNights;
          if (nights === null || nights === undefined || nights < 3) {
            errorsForTier.minNights = 'Must be >= 3';
          }
        }

        const minAdv = tier.conditions.minAdvanceBookingDays;
        if (minAdv !== null && minAdv !== undefined && minAdv < 0) {
          errorsForTier.minAdvanceBookingDays = 'Must be >= 0';
        }

        const maxN = tier.conditions.maxNights;
        if (maxN !== null && maxN !== undefined && maxN < 0) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    let payload: DiscountRuleCreateRequest | DiscountRuleUpdateRequest;
    ...
    if (!editingItem) {
      payload = {
        ...
        appliedRoomTypeIds: formData.appliedRoomTypeIds,
        status: formData.status,
        tiers: formData.tiers.map(t => ({
          discountType: t.discountType,
          discountValue: t.discountValue,
          conditions: {
            minNights: isLongStay ? (t.conditions.minNights ? Number(t.conditions.minNights) : null) : null,
            maxNights: t.conditions.maxNights ? Number(t.conditions.maxNights) : null,
            minAdvanceBookingDays: t.conditions.minAdvanceBookingDays ? Number(t.conditions.minAdvanceBookingDays) : null,
                  ...
                  <label 
                    ...
                    className={`flex items-center space-x-2.5 p-2 rounded border cursor-pointer select-none text-xs transition-colors
                      ${formData.appliedRoomTypeIds.includes(hrt.id)
                        ? 'border-red-600 bg-red-50/20 text-neutral-955 font-bold'
          ...
          {!editingItem ? (
            <div className="space-y-4 pt-2 border-t border-neutral-100">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-extrabold text-neutral-955 uppercase tracking-wide">
