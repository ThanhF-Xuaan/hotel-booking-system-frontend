...
const DiscountRulePage = () => {
  ...
  const fetchDiscountRules = async (roomTypeId) => {
    setIsLoading(true);
    setFetchError(null);
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to fetch discount rules.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      console.error(err);
      setFetchError(err.response?.data?.message || 'Failed to delete discount rule.');
    } finally {
      setIsLoading(false);
    }
  };
